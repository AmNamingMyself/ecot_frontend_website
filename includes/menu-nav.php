

<!--************************************
			Wrapper Start
	*************************************-->
	<div id="tg-wrapper" class="tg-wrapper">
		<!--************************************
				Header Start
		*************************************-->
		<header id="tg-header" class="tg-header tg-haslayout">
			<div class="tg-topbar">
				<div class="tg-leftbox">
					<span id="tg-datebox" class="tg-datebox"></span>
					<span class="tg-rtltextbox"> <a href="#">Students</a> <a href="#">Faculty</a><a href="#">Alumni</a> <a href="#">A-Z</a></span>
				</div>
				<div class="tg-rightbox">
					<span class="tg-tollfree"><a href="">Register</a> <a href="">Login</a></span>
					<div class="tg-themedropdown tg-languagesdropdown">
						<a href="javascript:void(0);" id="tg-languages" class="tg-btndropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
							
							<span>QUICKLINKS</span>
						</a>
						<div class="tg-themedropdownbox" aria-labelledby="tg-languages">
							<ul class="tg-languageslist">
								<li><span>Calendar</span></li>
								<li><span>Directory</span></li>
								<li><span>Academics</span></li>
								<li><span>Admissions</span></li>
							</ul>
						</div>
					</div>
				</div>
			</div>
			<div class="clearfix"></div>
			<div class="container">
				<div class="row">
					<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
						<div class="tg-logoandnoticeboard">
							<strong class="tg-logo"><a href=""><img src="images/logsd.png" alt="Eswatini College Of Technology"></a></strong>
							<div class="tg-noticeboard hidden-xs">
								<div class="tg-textbox">
									<span>Admission Opened!</span>
									<span>Last Date For Admission 02/03/2019</span>
								</div>
								<figure><img src="images/img-01.jpg" alt="image description"></figure>
							</div>
						</div>
						<div class="tg-navigationarea">
							<nav id="tg-nav" class="tg-nav">
								<div class="navbar-header">
									<button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#tg-navigation" aria-expanded="false">
										<span class="sr-only">Toggle navigation</span>
										<span class="icon-bar"></span>
										<span class="icon-bar"></span>
										<span class="icon-bar"></span>
									</button>
								</div>
								<div id="tg-navigation" class="collapse navbar-collapse tg-navigation">
									<ul>
										<li class="current-menu-item">
											<a href="index.php"><i class="fa fa-home"></i></a>
											
										</li>
										<li class="menu-item-has-mega-menu">
											<a href="javascript:void(0);">About</a>
											<div class="mega-menu">
												<ul class="mega-menu-row">
													<li class="mega-menu-col">
														<a href="javascript:void(0);">About ECOT</a>
														<ul>
															<li><a href="about.php">About</a></li>
															<li><a href="">History</a></li>
															<li><a href="">Governance</a></li>
														</ul>
													</li>
													<li class="mega-menu-col">
														<a href="javascript:void(0);">Media &amp; News</a>
														<ul>
															<li><a href="">Media Queries</a></li>
															<li><a href="">Events Calendar</a></li>
															<li><a href="">FAQ&acute;s</a></li>
														</ul>
													</li>
												</ul>
												<ul class="mega-menu-row">
													<li class="mega-menu-col">
														<figure><img src="images/imgz.png" alt="image description"></figure>
														<div class="tg-textbox">
															<strong>ESWATINI COLLEGE OF TECHNOLOGY</strong>
															<div class="tg-description">
																<p>ECOT&acute;s innovative and unique undergraduate and graduate programs are designed to meet your interests and career aspirations, along with the market-driven requirements of employers.  </p>
															</div>
															<a class="tg-btn tg-bgn-sm" href="javascript:void(0);">Apply Now</a>
														</div>
													</li>
												</ul>
											</div>
										</li>
										<li class="menu-item-has-mega-menu">
											<a href="javascript:void(0);">Academics</a>
											<div class="mega-menu tg-fullmegamenu">
												<ul class="mega-menu-row">
													<li class="mega-menu-col">
														<div class="tg-themetabs">
															<ul class="tg-themetabnav" role="tablist">
																<li role="presentation" class="active">
																	<a href="#tg-science" aria-controls="tg-science" role="tab" data-toggle="tab">Science</a>
																</li>
																<li role="presentation">
																	<a href="#tg-management" aria-controls="tg-management" role="tab" data-toggle="tab">Management</a>
																</li>
																<li role="presentation">
																	<a href="#tg-medical" aria-controls="tg-medical" role="tab" data-toggle="tab">Education</a>
																</li>
																<li role="presentation">
																	<a href="#tg-arts" aria-controls="tg-arts" role="tab" data-toggle="tab">Building</a>
																</li>
																<li role="presentation">
																	<a href="#tg-informationtechnology" aria-controls="tg-informationtechnology" role="tab" data-toggle="tab">Information Technology</a>
																</li>
																
															</ul>
															<div class="tab-content tg-themetabcontent">
																<div role="tabpanel" class="tab-pane tg-tabpane active" id="tg-science">
																	<strong>Engineering Science</strong>
																	<div class="tg-posts">
																		<div id="tg-navtabslider" class="tg-navtabslider tg-megamenuslider owl-carousel">
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/ele.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags" style="color:#000;">
																							<li><a href="javascript:void(0);">Electrical</a></li>
																							<li><a href="javascript:void(0);">Engineering</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventslist.html">National Diploma in Electrical Engineering</a></h3>
																						</div>
																						
																					</div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/mech.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Mechanical</a></li>
																							<li><a href="javascript:void(0);">Engineering</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventsgrid.html">National Diploma in Mechanical Engineering</a></h3>
																						</div>
																						
																					</div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/auto.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Automotive (Light)</a></li>
																							<li><a href="javascript:void(0);">Engineering</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventdetail.html">National Diploma in Automotive Engineering</a></h3>
																						</div>
																				  </div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/auto.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Automotive (Heavy)</a></li>
																							<li><a href="javascript:void(0);">Engineering</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventslist.html">National Diploma in Automotive Engineering</a></h3>
																						</div>
																				  </div>
																				</article>
																			</div>
																			
																			
																		</div>
																	</div>
																</div>
																<div role="tabpanel" class="tab-pane tg-tabpane" id="tg-management">
																	<strong>Business Administration</strong>
																	<div class="tg-posts">
																		<div id="tg-navtabslider" class="tg-navtabslider tg-megamenuslider owl-carousel">
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/ttc.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Tourism</a></li>
																							<li><a href="javascript:void(0);">Management</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventsgrid.html">B-tech Tourism</a></h3>
																						</div>
																				  </div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/it.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Human Resource</a></li>
																							<li><a href="javascript:void(0);">Management</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventdetail.html">B-Tech HRM</a></h3>
																						</div>
																					
																					</div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/dnt.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Office</a></li>
																							<li><a href="javascript:void(0);">Management</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventslist.html">Office Management</a></h3>
																						</div>
																				  </div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/dnt.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Internal </a></li>
																							<li><a href="javascript:void(0);">Auditing</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventsgrid.html">B-tech in Internal Auditing</a></h3>
																						</div>
																						
																					</div>
																				</article>
																			</div>
																			
																		</div>
																	</div>
																</div>
																<div role="tabpanel" class="tab-pane tg-tabpane" id="tg-medical">
																	<strong>Education</strong>
																	<div class="tg-posts">
																		<div id="tg-navtabslider" class="tg-navtabslider tg-megamenuslider owl-carousel">
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/ttc.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Commercial</a></li>
																							<li><a href="javascript:void(0);">Teaching</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventdetail.html">Diploma in Commercial Teaching</a></h3>
																						</div>
																				  </div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/it.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Design &amp;</a></li>
																							<li><a href="javascript:void(0);">Technology</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventslist.html">Diploma in Design &amp; Technology</a></h3>
																						</div>
																				  </div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/ttc.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">ICT</a></li>
																							<li><a href="javascript:void(0);">Education</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventsgrid.html">Diploma in ICT Education</a></h3>
																						</div>
																				  </div>
																				</article>
																			</div>
																			
																		</div>
																	</div>
																</div>
																<div role="tabpanel" class="tab-pane tg-tabpane" id="tg-arts">
																	<strong>Building &amp; Civil</strong>
																	<div class="tg-posts">
																		<div id="tg-navtabslider" class="tg-navtabslider tg-megamenuslider owl-carousel">
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/civil.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Civil</a></li>
																							<li><a href="javascript:void(0);">Engineering</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventslist.html">Civil Engineering</a></h3>
																						</div>
																						</div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/dbs.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Building</a></li>
																							<li><a href="javascript:void(0);">Studies</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventsgrid.html">Diploma in Building Studies</a></h3>
																						</div>
																						</div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/qs.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Quantity</a></li>
																							<li><a href="javascript:void(0);">Survey</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventdetail.html">Diploma in Quantity Survey</a></h3>
																						</div>
																						</div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/civil.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Architectural</a></li>
																							<li><a href="javascript:void(0);">Design</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventdetail.html">Diploma in Architecture</a></h3>
																						</div>
																						</div>
																				</article>
																			</div>
																			
																		</div>
																	</div>
																</div>
																<div role="tabpanel" class="tab-pane tg-tabpane" id="tg-informationtechnology">
																	<strong>Information Technology </strong>
																	<div class="tg-posts">
																		<div id="tg-navtabslider" class="tg-navtabslider tg-megamenuslider owl-carousel">
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/it.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Computer</a></li>
																							<li><a href="javascript:void(0);">Science</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventslist.html">Diploma in Computer Science</a></h3>
																						</div>
																						</div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/it.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">Information</a></li>
																							<li><a href="javascript:void(0);">Technology</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventsgrid.html">Diploma in Information Technology</a></h3>
																						</div>
																						</div>
																				</article>
																			</div>
																			<div class="item">
																				<article class="tg-themepost">
																					<figure class="tg-featuredimg">
																						<a href="javascript:void(0);">
																							<img src="images/themepost/it.jpg" alt="image description">																						</a>																					</figure>
																					<div class="tg-themepostcontent">
																						<ul class="tg-themeposttags">
																							<li><a href="javascript:void(0);">B-tech</a></li>
																							<li><a href="javascript:void(0);">ICT</a></li>
																						</ul>
																						<div class="tg-themeposttitle">
																							<h3><a href="eventdetail.html">B-tech in Information Communications</a></h3>
																						</div>
																						</div>
																				</article>
																			</div>
																			
																		</div>
																	</div>
																</div>
								
															
													</li>
												</ul>
											</div>
										</li>
										
										<li class="menu-item-has-children">
											<a href="javascript:void(0);">Admission</a>
											<ul class="sub-menu">
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Apply </a>
													<ul class="sub-menu">
														<li><a href="">Apply Now</a></li>
														<li><a href="">Proficiency</a></li>
														<li><a href="">Considerations</a></li>
														<li><a href="">Requirements</a></li>
														<li><a href="">FAQ&acute;s</a></li>
													</ul>
												</li>
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Programs</a>
													<ul class="sub-menu">
														<li><a href="">Graduate</a></li>
														<li><a href="">Undergraduate</a></li>
													</ul>
												</li>
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Tuitions</a>
													<ul class="sub-menu">
														<li><a href="">Self Sponsored</a></li>
														<li><a href="">Government Sponsored</a></li>
														
													</ul>
												</li>
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Careeer Guidance</a>
													<ul class="sub-menu">
														<li><a href="">Career Centre</a></li>
														<li><a href="">Career Fairs</a></li>
														<li><a href="">Get Help</a></li>
													</ul>
												</li>
											</ul>
										</li>
										<li class="menu-item-has-children">
											<a href="javascript:void(0);">Student Life</a>
											<ul class="sub-menu">
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Academic Advising </a>
													<ul class="sub-menu">
														<li><a href="">Academic Excellence</a></li>
														<li><a href="">Student Responsibilities</a></li>
														<li><a href="">Considerations &amp; FAQ&acute;s</a></li>
													</ul>
												</li>
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Academic Calendar</a>
													<ul class="sub-menu">
														<li><a href="">Events</a></li>
														<li><a href="">Full Calendar</a></li>
														<li><a href="">News &amp; Media</a></li>
													</ul>
												</li>
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Career Centre</a>
													<ul class="sub-menu">
														<li><a href="">Job Search</a></li>
														<li><a href="">Interview Help</a></li>
														<li><a href="">Career Counciling</a></li>
														<li><a href="">Alumni Help</a></li>
														
													</ul>
												</li>
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Campus Services</a>
													<ul class="sub-menu">
														<li><a href="">Housing</a></li>
														<li><a href="">Nutrition</a></li>
														<li><a href="">Security</a></li>
														<li><a href="">Recreation</a></li>
													</ul>
												</li>
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Student Organization</a>
													<ul class="sub-menu">
														<li><a href="">Student Associations</a></li>
														<li><a href="">Student Representative</a></li>
														<li><a href="">Student Clubs</a></li>
														<li><a href="">Student Societies</a></li>
													</ul>
												</li>
											</ul>
										</li>
										<li class="menu-item-has-children">
											<a href="javascript:void(0);">Research</a>
											<ul class="sub-menu">
												<li><a href="">Library</a></li>
												<li class="menu-item-has-children">
													<a href="javascript:void(0);">Research</a>
													<ul class="sub-menu">
														<li><a href="">Interlectual Property</a></li>
														<li><a href="">Student&acute;s Resources</a></li>
														<li><a href="">Assistive Technology</a></li>
													</ul>
												</li>
											</ul>
										</li>
										<li class="menu-item-has-children">
											<a href="javascript:void(0);">Alumni</a>
											<ul class="sub-menu">
												<li><a href="">Join</a></li>
												<li><a href="">Alumni Programs</a></li>
												<li><a href="">Mentoring &amp; Giving</a></li>
											</ul>
										</li>
										<li class="menu-item-has-children">
											<a href="javascript:void(0);">Giving</a>
											<ul class="sub-menu">
												<li><a href="">Donate Now</a></li>
												<li><a href="">Ways to give</a></li>
												<li><a href="">Your impact</a></li>
											</ul>
										</li>
										<li class=""><a href="javascript:void(0);">Vacancies</a></li>
										<li class=""><a href="javascript:void(0);">Contacts</a></li>
									</ul>
								</div>
							</nav>
							<div class="tg-searchbox">
								<a id="tg-btnsearch" class="tg-btnsearch" href="javascript:void(0);"><i class="icon-magnifier"></i></a>
								<form class="tg-formtheme tg-formsearch">
									<fieldset><input type="search" name="search" class="form-control" placeholder="Start Your Search Here"></fieldset>
								</form>
							</div>
						</div>
					</div>
				</div>
			</div>
		</header>
		<!--************************************
				Header End
		*************************************-->